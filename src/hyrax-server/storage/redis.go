...
// init sets up NUM_CONNS routines that will each handle incoming commands for
// their channel
func init() {
...
// CmdPretty is a wrapper around Cmd, it takes in a command and a variadic list
// of arguments for that command. Used for commands defined directly in hyrax's
// code, where Cmd is used for commands that are being invoked by a client
// (generally)
...
// determines the type of the return, decoding it, as well figuring out if
...
func Cmd(cmd []byte, args []interface{}) (interface{}, error) {
	...
	case redis.MultiReply:
		//I have a merge request with radix for a proper ListBytes() method.
		//Hopefully the guy notices, if not I'll change the radix dependency to
		//be my fork
		l, err := r.List()
		if err != nil {
			return nil, err
