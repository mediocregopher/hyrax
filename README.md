...
All commands can take in an optional field `quiet`, which, if true, will silence any `mon-push` messages that would
be generated by a command. The default is `false`.

Example:
```json
{ "command":"set", "payload":{ "domain":"td","id":"tid","secret":"lotsahex","values":["tv"] }, "quiet":true }
{ "command":"set", "return":"OK"}
```
